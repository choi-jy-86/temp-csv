...
namespace EditorGUITable
{
	...
	public static class GUITable
	{
			...
			params GUITableOption[] options) 
		{
			...
			if( rows.Count != collectionProperty.arraySize )
			{
				// Reorderable と Filter は併用できない
				options = options
					.Where( content => content.type != GUITableOption.Type.Reorderable )
					.ToArray();
			}
			...
			params GUITableOption[] options)
		{
			...
			{
				tableState.SetReorderableList( null );
			}
